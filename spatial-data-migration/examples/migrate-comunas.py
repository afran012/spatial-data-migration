...
def create_athena_table(athena_client, database_name, table_name, s3_location):
    """Crea la tabla en Athena con la estructura de comunas y corregimientos"""
...
def migrate_comunas(table_name='comunas_y_corregimientos'):
    """Migración de datos de comunas y corregimientos"""
    try:
        print("\n=== Iniciando migración de comunas y corregimientos ===")
        ...
        with engine.connect() as conn:
            result = conn.execute(text('SELECT COUNT(*) FROM shapes.comunas_y_corregimientos'))
            original_count = result.scalar()
            
            result = conn.execute(text('SELECT COUNT(DISTINCT id) FROM shapes.comunas_y_corregimientos'))
        ...
        SELECT DISTINCT ON (id)
            ...
            "Shape_Leng",
            "Shape_Area"
        FROM shapes.comunas_y_corregimientos
        ORDER BY id
        """
        ...
        df_final = pd.DataFrame({
            ...
            'codigo': gdf.codigo,
            'nombre': gdf.nombre,
            'identifica': gdf.identifica,
            'limitemuni': gdf.limitemuni,
            'subtipo_co': gdf.subtipo_co,
            'fecha_actu': gdf.fecha_actu.astype(str),
        ...
        local_path = f"temp_comunas_{timestamp}.parquet"
...
if __name__ == "__main__":
    ...
    import argparse
    parser = argparse.ArgumentParser(description='Migrar datos de comunas y corregimientos a Athena')
    parser.add_argument('--table-name', type=str, default='comunas_y_corregimientos',
                      help='Nombre de la tabla en Athena')
    args = parser.parse_args()
    
    migrate_comunas(args.table_name)